...
/*
   1. noCache 不使用缓存，全部走网络
    ...
    private Api(final int hostType) {
        ...
        //缓存
        File cacheFile = new File(BegoitMoocApplication.Companion.getContextInstance().getCacheDir(), "cache");
        //100Mb
        ...
        Interceptor headerInterceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                Request.Builder builder = chain.request().newBuilder()
                        ...
                        .addHeader("tag", "Android_BGJW")
                        .addHeader("token", HeaderConfig.getToken());
                //Log.e("token",HeaderConfig.token);
...
//                .addConverterFactory(GsonConverterFactory.create(gson))
    ...
    public static String getCacheControl() {
        return NetWorkUtils.isNetConnected(BegoitMoocApplication.Companion.getContextInstance()) ? CACHE_CONTROL_AGE : CACHE_CONTROL_CACHE;
    }
    ...
    private final Interceptor mRewriteCacheControlInterceptor = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!NetWorkUtils.isNetConnected(BegoitMoocApplication.Companion.getContextInstance())) {
                    ...
                    if (contentLength != 0) {
                        ...
                        if (response.code() != 404) {
                            try {
                                ...
                                //token 超时
                                if(401 == JSON.parseObject(result).getIntValue("status")){
                                    Looper.prepare();
                                    if (request.url().toString().contains("channel/courseStudy")){
                                        StudyLogUploadUtil.INSTANCE.setDoUpload(false);
                                    }
                                    ActivityUtils.logout(JSON.parseObject(result).getString("msg"));
                                    Looper.loop();
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
