...
/**
 * @Description:考核试题回答正确与否状态展示
 * @Author:gxj
 * @Time 2019/4/25
 */

public class ShowTestAnswerStatusAdapter extends BaseQuickAdapter<VideoTestEntity,BaseViewHolder> {
    private int position = 1;
    private OnItemClickListener onItemClickListener;

    public ShowTestAnswerStatusAdapter(@Nullable List<VideoTestEntity> data, Context mContext) {
    ...
    protected void convert(BaseViewHolder helper, final VideoTestEntity item) {
        if (item.answer.equals(item.userAnswer)) {
            helper.setBackgroundRes(R.id.tv_position, R.drawable.bg_green_round_no_line);
            helper.setTextColor(R.id.tv_position, Color.parseColor("#ffffff"));
        }else {
            helper.setBackgroundRes(R.id.tv_position, R.drawable.bg_white_round_yellow_line);
            helper.setTextColor(R.id.tv_position, Color.parseColor("#EC6743"));
        }
